...
function IBMCloudEnv() {
	...
	// we keep an in-memory map where key is the value of mappingsFilePath and
	// the value is the loaded JSON object from contents of mappingsFilePath
	const mappings = {};
	...
	function init(mappingsFilePath) {
		...
		if (!mappings.mappingsFilePath) {
			logger.trace("Initializing with", mappingsFilePath);
			if (!fs.existsSync(mappingsFilePath)) {
				logger.warn("File does not exist", mappingsFilePath);
				return;
			}
		
			mappings.mappingsFilePath = JSON.parse(fs.readFileSync(mappingsFilePath, "UTF8"));
		}
		_.each(mappings.mappingsFilePath, function (value, key) {
