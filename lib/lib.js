...
function IBMCloudEnv(){
	...
	function init(mappingsFilePath){
		...
		mappingsFilePath = mappingsFilePath || "/server/config/ibm-cloud-env-config.json";
		...
		var mappingsJson = JSON.parse(fs.readFileSync(mappingsFilePath, "UTF8"));
	...
	function processMapping(mappingName, config) {
		...
		config.searchPatterns.every(function (searchPattern) {
			...
			var value = processSearchPattern(mappingName, searchPattern);
			if (value) {
	...
	function processSearchPattern(mappingName, searchPattern){
		...
		var patternComponents = searchPattern.split(":");

		switch (patternComponents[0]){
			case PREFIX_PATTERN_FILE:
				return processFileSearchPattern(patternComponents); break;
			case PREFIX_PATTERN_CF:
				return processCFSearchPattern(patternComponents); break;
			case PREFIX_PATTERN_ENV:
				return processEnvSearchPattern(patternComponents); break;
			default:
	...
	function processFileSearchPattern(patternComponents){
		...
		var filePath = process.cwd() + patternComponents[1];
	...
	function processCFSearchPattern(patternComponents){
		...
		var vcapApplicationString = process.env.VCAP_APPLICATION;
		...
		if (patternComponents[1][0] === "$"){
			...
			var value = processJSONPath(vcapServicesString, patternComponents[1]);
			...
			var jsonPath = '$..[?(@.name=="' + patternComponents[1] + '")].credentials';
	...
	function processEnvSearchPattern(patternComponents){
		...
		var value = process.env[patternComponents[1]];
		if (patternComponents.length === 3){
